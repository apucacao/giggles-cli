...
async fn prepare_import(source: Option<&str>, config: &Config, client: &Client) -> Result<String> {
    let res = client
        .post(format!("{}/api/import/prepare", config.server))
        .bearer_auth(&config.api_key)
        .json(&PrepareRequest { source })
        .send()
        .await
        .context("Failed to call /api/import/prepare")?;
...
async fn run_pipeline(
    ...
    if verbose {
        shell::status("Preparing", "import");
    }
    let short_id = prepare_import(resolved.source.as_deref(), config, client)
        .await
...
pub async fn import_one(input: &str, config: &Config) -> Result<String> {
    let client = Client::builder()
        .timeout(Duration::from_secs(300))
        .build()
        .context("Failed to create HTTP client")?;

    run_pipeline(input, config, &client, true)
        .await
        .map_err(|e| match e {
...
pub async fn import_batch(inputs: Vec<String>, config: Config, concurrency: usize) {
    ...
    for input in inputs {
        ...
        handles.push(tokio::spawn(async move {
            ...
            let outcome = match run_pipeline(&input, &cfg, &cli, false).await {
                Ok(_) => ItemOutcome::Imported,
            ...
            let line = match &outcome {
                ItemOutcome::Imported => {
                    shell::format_status("Imported", format!("{input} ({elapsed:.1}s)"))
                }
    ...
    let imported = results
        .iter()
        .filter(|(_, o)| matches!(o, ItemOutcome::Imported))
        .count();
    let skipped = results
        .iter()
    ...
    shell::status(
        "Finished",
        format!(
            "in {time_str} — {imported} imported, {skipped} skipped, {} failed",
            failed.len()
        ),
    );
